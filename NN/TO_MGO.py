...
class STO(nn.Module):
    ...
    def forward(self,fs0,fs1,fs2):
        ...
        for i in range(1,self.T//2+1):
            if i==1:
                offset_0.append(self.s0_opns[i-1](fs0[0],fs0[1]))
            else:
                offset_0.append(self.s0_opns[i-1](fs0[0],fs0[i],((i/(i-1))*offset_0[i-2])/2))
...
class TO_MGO(nn.Module):
    def __init__(self,T):
        super(TO_MGO,self).__init__()
        print('NN: ',self.__class__.__name__)
